...
public class OAuth2Service {
    ...
    private final Map<String, ConsentRecord> userConsents =
        new ConcurrentHashMap<>();
    private final Map<String, AuthCodeRecord> authorizationCodes =
        new ConcurrentHashMap<>();
    private final Map<String, AccessTokenRecord> accessTokens =
        new ConcurrentHashMap<>();
    ...
    public Map<String, Object> exchangeAuthorizationCode(
        ...
        String customToken = firebaseAuthService.createCustomToken(
            authCodeRecord.getUserId()
        );

        // Get Firebase token expiry and calculate OAuth2 token expiry (60 seconds less for safety)
        long firebaseTokenExpirySeconds =
            firebaseAuthService.getCustomTokenExpirySeconds(customToken);
        long oauthTokenExpirySeconds = Math.max(
            60,
            firebaseTokenExpirySeconds - 60
        ); // At least 60 seconds, but 60 seconds less than Firebase token
    ...
    public Map<String, Object> refreshAccessToken(
        ...
        if (userInfo[0] == null) {
            throw new IllegalArgumentException(
                "Invalid refresh token - no associated user found"
            );
        }
        ...
        long firebaseTokenExpirySeconds =
            firebaseAuthService.getCustomTokenExpirySeconds(newFirebaseToken);
        long oauthTokenExpirySeconds = Math.max(
            60,
            firebaseTokenExpirySeconds - 60
        );
        ...
        String newAccessToken = generateJwtAccessToken(
            userId,
            clientId,
            scope,
            newFirebaseToken,
            oauthTokenExpirySeconds
        );
            ...
            accessTokens
                ...
                .removeIf(
                    record ->
                        ...
                        record.getClientId().equals(clientId)
                );
        }
    ...
    private String generateJwtAccessToken(
        ...
        System.out.println(
            "generateJwtAccessToken called for user: " +
                userId +
                ", firebase_token: " +
                (firebaseToken != null
                    ? firebaseToken.substring(
                          0,
                          Math.min(50, firebaseToken.length())
                      ) +
                      "..."
                    : "null")
        );
    ...
    public String createJwtAccessToken(
        ...
        System.out.println(
            "createJwtAccessToken called for user: " +
                userId +
                ", firebase_token: " +
                (firebaseToken != null
                    ? firebaseToken.substring(
     ...
     */
    public boolean verifyPkceChallenge(
        String codeVerifier,
