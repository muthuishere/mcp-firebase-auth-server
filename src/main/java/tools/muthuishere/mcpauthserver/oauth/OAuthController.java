...
public class OAuthController {
    ...
    private final Map<String, AuthorizationCodeInfo> authorizationCodes =
        new ConcurrentHashMap<>();

    // Store registered clients temporarily (in production, use database)
    private final Map<String, RegisteredClientInfo> registeredClients =
        new ConcurrentHashMap<>();
    ...
    public String authorize(
        ...
        if (!client.getRedirectUris().contains(redirect_uri)) {
            ...
            if (isValidRedirectUri(redirect_uri)) {
                ...
                java.util.List<String> updatedUris = new java.util.ArrayList<>(
                    client.getRedirectUris()
                );
                updatedUris.add(redirect_uri);
                client.setRedirectUris(updatedUris);
                System.out.println(
                    "Added new redirect_uri to client " +
                        client_id +
                        ": " +
                        redirect_uri
                );
                ...
                return (
                    "redirect:/error?error=invalid_request&error_description=" +
                    URLEncoder.encode(
        ...
        String authenticatedUserId = (String) request
            .getSession()
            .getAttribute("authenticated_user_id");
        String authenticatedUserEmail = (String) request
            .getSession()
            .getAttribute("authenticated_user_email");
        String authenticatedUserName = (String) request
            .getSession()
            .getAttribute("authenticated_user_name");

        // If user is not authenticated or session data is incomplete, redirect to login
        if (
            authenticatedUserId == null ||
            authenticatedUserId.trim().isEmpty() ||
        ...
        model.addAttribute(
            "client_name",
            client.getClientName() != null ? client.getClientName() : client_id
        );
        model.addAttribute("redirect_uri", redirect_uri);
        model.addAttribute("scope", scope != null ? scope : "read");
        model.addAttribute("state", state);
    ...
    public String handleConsent(
        ...
        try {
            ...
            AuthorizationRequest authRequest = (AuthorizationRequest) request
                .getSession()
            ...
            String sessionCsrfToken = (String) request
                .getSession()
            ...
            if (
                csrf_token == null ||
                !csrf_token.equals(sessionCsrfToken) ||
                !csrfTokens.containsKey(csrf_token)
            ) {
                return (
                    "redirect:/error?error=invalid_request&error_description=" +
                    URLEncoder.encode(
            ...
            boolean isDenied =
                "deny".equals(action) || "false".equals(approved);
            ...
            if (firebase_token == null) {
                firebase_token = (String) request
                    .getSession()
                    .getAttribute("firebase_token");
                if (firebase_token != null) {
                    System.out.println(
                        "Using Firebase token from session: " +
                            firebase_token.substring(
                                0,
                                Math.min(50, firebase_token.length())
                            ) +
                            "..."
                    );
                }
            ...
            if (firebase_token == null) {
                try {
                    ...
                    System.out.println(
                        "Generated new Firebase custom token for user: " + uid
                    );
                } catch (Exception e) {
                    System.out.println(
                        "Failed to generate Firebase token for user " +
                            uid +
                            ": " +
                            e.getMessage()
                    );
                    return (
                        "redirect:/error?error=server_error&error_description=" +
                        URLEncoder.encode(
            ...
            codeInfo.setCodeChallengeMethod(
    ...
    private Map<String, Object> baseTokenResponse(
        String accessToken,
        ...
        resp.put("expires_in", expiresInSeconds);
        if (refreshToken != null) resp.put("refresh_token", refreshToken);
        return resp;
    }
    ...
    public ResponseEntity<?> token(
        ...
        try {
            ...
            if (
                codeInfo == null ||
                codeInfo.getExpiresAt() < System.currentTimeMillis()
            ) {
            ...
            if (
                !codeInfo.getClientId().equals(client_id) ||
                !codeInfo.getRedirectUri().equals(redirect_uri)
            ) {
                Map<String, String> errorResponse = new HashMap<>();
                errorResponse.put("error", "invalid_grant");
                errorResponse.put(
            ...
            if (codeInfo.getCodeChallenge() != null) {
                ...
                if (
                    !oauth2Service.verifyPkceChallenge(
                        code_verifier,
                        codeInfo.getCodeChallenge(),
                        codeInfo.getCodeChallengeMethod()
                    )
                ) {
                    Map<String, String> errorResponse = new HashMap<>();
                    errorResponse.put("error", "invalid_grant");
            ...
            tokenResponse.put(
                "expires_in",
                oauth2Service.getAccessTokenExpirySeconds()
            );
    ...
    public ResponseEntity<?> registerClient(
        ...
        try {
            ...
            java.util.List<String> redirectUris = (java.util.List<
                String
            >) requestBody.get("redirect_uris");
            String tokenEndpointAuthMethod = (String) requestBody.getOrDefault(
                "token_endpoint_auth_method",
                "client_secret_basic"
            );
            @SuppressWarnings("unchecked")
            java.util.List<String> grantTypes = (java.util.List<
                String
            >) requestBody.getOrDefault(
                "grant_types",
                java.util.List.of("authorization_code")
            );
            @SuppressWarnings("unchecked")
            java.util.List<String> responseTypes = (java.util.List<
                String
            >) requestBody.getOrDefault(
                "response_types",
    ...
    public ResponseEntity<?> userInfo(HttpServletRequest request) {
        try {
            ...
            Map<String, Object> userInfo =
                oauth2Service.extractUserInfoFromToken(accessToken);
            if (userInfo == null) {
                Map<String, String> errorResponse = new HashMap<>();
                errorResponse.put("error", "invalid_token");
                errorResponse.put(
