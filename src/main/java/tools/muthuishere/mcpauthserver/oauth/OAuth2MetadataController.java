...
public class OAuth2MetadataController {
    ...
    public ResponseEntity<Map<String, Object>> wellKnownConfiguration() {
        ...
        config.put("authorization_endpoint", serverUrlsConfig.getAuthorizationEndpoint());
        ...
        config.put("scopes_supported", new String[]{"openid", "profile", "email", "firebase"});
        config.put("token_endpoint_auth_methods_supported", new String[]{"client_secret_basic", "client_secret_post", "none"});
        config.put("claims_supported", new String[]{"sub", "aud", "iss", "exp", "iat", "email", "email_verified", "firebase"});
        ...
        Map<String, Object> jwks = Map.of(
            "keys", new Object[]{
                Map.of(
                    "kty", "RSA",
                    "use", "sig",
                    "kid", "firebase-auth-proxy-key",
                    "alg", "RS256",
                    "n", "placeholder-for-actual-key",
                    "e", "AQAB"
                )
            }
        );
    ...
    public ResponseEntity<Map<String, Object>> authorizationServerMetadata() {
        ...
        metadata.put("response_types_supported", new String[]{"code", "token"});
        ...
        metadata.put("registration_endpoint", serverUrlsConfig.getRegistrationEndpoint());
    ...
    public ResponseEntity<Map<String, Object>> urlsConfig() {
        ...
        config.put("registrationEndpoint", serverUrlsConfig.getRegistrationEndpoint());
        config.put("userInfoEndpoint", serverUrlsConfig.getUserInfoEndpoint());
        config.put("jwksUri", serverUrlsConfig.getJwksUri());
        config.put("wellKnownEndpoint", serverUrlsConfig.getWellKnownEndpoint());
